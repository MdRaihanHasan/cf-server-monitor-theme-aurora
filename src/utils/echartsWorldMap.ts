...
// World map data sources: fall back across multiple CDNs so a single CDN outage does not break the map
...
const WORLD_MAP_CACHE_KEY_PREFIX = 'cf-server-monitor-theme-aurora:world-map'
...
async function loadWorldGeoJson(): Promise<WorldGeoJson> {
  ...
  for (const url of WORLD_GEO_JSON_URLS) {
    try {
      ...
      console.warn(`Failed to load world map: ${url}`, error)
    }
  }
