...
// Mapping from region emoji to names
...
export function isRegionMatch(regionEmoji: string, searchTerm: string): boolean {
  ...
  if (!regionInfo) {
    // Not in the mapping table, fall back to a simple substring match
...
/**
 * Get the display name of a region
 * @param regionEmoji Region emoji
 * @param language Language ('en' | 'zh')
 * @returns Region name
 */
export function getRegionDisplayName(regionEmoji: string, language: 'en' | 'zh' = 'en'): string {
  const regionInfo = emojiToRegionMap[regionEmoji]
  if (!regionInfo) {
    return regionEmoji
  }
...
/**
 * Get the list of all supported region emojis
 ...
 * Get the region code
 * @param regionEmoji Region emoji
 * @returns Region code (e.g. HK, CN, US)
 */
...
/**
 * Get the emoji for a region code
 * @param code Region code (e.g. HK, CN, US)
 * @returns Region emoji
 */
...
/**
 * Search for regions matching a search term
 * @param searchTerm Search term
 * @returns Array of matching region info
 */
...
/**
 * Get region info by alias or code
 * @param aliasOrCode Alias or code (e.g. hk, HK, 香港, hongkong)
...
/**
 * Extract region emojis from text
 * @param text Text containing region emojis
 * @returns Array of extracted region emojis
 */
export function extractRegionEmojis(text: string): string[] {
  const emojis: string[] = []
  // Match flag emojis (composed of two regional indicator characters)
