...
const device = ref('Detecting')
const browser = ref('Detecting')
const ip = ref('Loading')
const isp = ref('Loading')
const location = ref('Locating visitor')
const countryCode = ref('')
const visitTime = ref(formatVisitTime(new Date()))
const flagVisible = ref(true)
const expand = ref(false)

const subtitle = computed(() => loading.value ? 'Detecting' : location.value || 'Web visitor')
  ...
  const loaders = [
    async (): Promise<VisitorGeoData> => {
      ...
      return {
        ...
        location: [data.country, data.city || data.region].filter(Boolean).join(' · ') || 'Unknown location',
    ...
    async (): Promise<VisitorGeoData> => {
      ...
      return {
        ip: data.ip,
        isp: data.asn?.org || data.company?.name || data.datacenter?.datacenter || data.asn?.descr || 'Unknown ISP',
        location: [
    ...
    async (): Promise<VisitorGeoData> => {
      ...
      return {
        ...
        location: [data.country_name, data.city || data.region].filter(Boolean).join(' · ') || 'Unknown location',
        countryCode: data.country_code || '',
      }
    },
    async (): Promise<VisitorGeoData> => {
      ...
      return {
        ...
        location: [data.data.country, data.data.city || data.data.province].filter(Boolean).join(' · ') || 'Unknown location',
...
onMounted(async () => {
  ...
  if (geo) {
    ...
    isp.value = 'Network info unavailable'
    location.value = 'Web visitor'
  }
    ...
    >
      ...
      >
        ...
        >
          ...
          <div
            v-else
            class="flex h-4 w-4 shrink-0 items-center justify-center rounded bg-teal-500/10 text-teal-600"
          >
