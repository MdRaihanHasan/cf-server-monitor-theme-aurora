...
// Use the shared RPC instance to avoid creating duplicate connections
...
// Optimized chart color palette (used when there are multiple tasks)
const chartColors = [
  '#FF6B6B', // coral red
  '#4ECDC4', // teal
  '#A78BFA', // violet
...
// Get the task color (based on the task's index in the full list)
...
// Prefer raw samples within the current query period; only fall back to the API's aggregated fields when they are missing.
...
// Toggle a task's selected state
...
const pingChartOption = computed(() => {
  ...
  // Build the series, ensuring colors match the cards
  ...
  // Color mapping table (used for the tooltip)
  ...
  return {
    animation: false,
    // Global color settings (used for the legend, etc.)
    ...
    tooltip: {
      ...
      formatter: (params: unknown) => {
        ...
        // Sort by latency value for display
        const sortedParams = [...p].sort((a, b) => (a.value ?? 0) - (b.value ?? 0))

        for (const item of sortedParams) {
          if (item.value !== null && item.value !== undefined) {
            // Find the matching task ID by task name, then get its color
...
<template>
  <div class="flex flex-col gap-4">
    ...
    <Tabs v-model="selectedView" class="w-full items-center">
      ...
      <div class="flex gap-2 items-center">
        <Button
          ...
          :class="[selectedTaskIds.length === tasks.length && 'bg-background !text-teal-600']"
          @click="showAllTasks"
        >
          Select All
        </Button>
        <Button
          variant="ghost" size="xs" class="h-7 rounded-sm border-none bg-background/60 hover:bg-background"
          :class="[!selectedTaskIds.length && 'bg-background !text-teal-600']"
          @click="hideAllTasks"
        >
          Deselect All
        </Button>
    ...
    <Spinner :show="loading" content-class="flex flex-col gap-4">
      ...
      <template v-else>
        <!-- Latest-value stat cards (click to toggle selected state) -->
        <div
        ...
        >
          <div
            ...
            class="flex cursor-pointer select-none items-center gap-3 rounded-md p-2 transition-all bg-background/60 hover:bg-background hover:shadow-[0_0_0_1px] hover:shadow-teal-600/10"
            :class="[
          ...
          >
            <div class="flex-1 min-w-0">
              ...
              <div class="text-xs mt-1 flex gap-1.5 items-center text-muted-foreground">
                <span class="font-medium" title="Average latency">
                  {{ task.avg !== undefined ? `${Math.round(task.avg)}ms` : '-' }}
                </span>
                <template v-if="task.loss !== undefined">
                  <span class="opacity-60">·</span>
                  <span title="Packet loss">{{ task.loss.toFixed(2) }}%</span>
                </template>
                <template v-if="task.p99_p50_ratio !== undefined">
                  <span class="opacity-60">·</span>
                  <span title="Jitter">{{ task.p99_p50_ratio.toFixed(2) }}</span>
                </template>
              </div>
            </div>
          </div>
        ...
        <div class="flex flex-wrap gap-2 items-center py-2">
          <!-- Latency visualization toggle -->
          <Button
            variant="ghost" size="xs" class="h-7 rounded-sm border-none bg-background/60 hover:bg-background"
            :class="[showDelay && 'bg-background !text-teal-600']" @click="showDelay = !showDelay"
          >
            Latency
          </Button>
          <!-- Packet loss visualization toggle -->
          <Button
            variant="ghost" size="xs" class="h-7 rounded-sm border-none bg-background/60 hover:bg-background"
            :class="[showLoss && 'bg-background !text-teal-600']" @click="showLoss = !showLoss"
          >
            Loss
          </Button>
          <!-- Peak smoothing toggle -->
          <div class="flex gap-2 items-center">
            <Button
              variant="ghost" size="xs" class="h-7 rounded-sm border-none bg-background/60 hover:bg-background"
              :class="[cutPeak && 'bg-background !text-teal-600']" @click="cutPeak = !cutPeak"
            >
              Smooth Peaks
            </Button>
            <DataTooltip
              content="Smooths data with the EWMA algorithm and filters out sudden spikes"
              placement="top"
