...
import { formatOfflineTime, getCustomTags, getTrafficUsed, getTrafficUsedPercentage, hasRegion, showTrafficProgress } from '@/utils/nodeHelper'
...
const columns: ColumnConfig[] = [
  { key: 'status', label: 'Status', width: '40px', sortable: true },
  { key: 'os', label: 'OS', width: '40px', sortable: true },
  { key: 'name', label: 'Node', width: 'minmax(150px, 0.8fr)', sortable: true },
  { key: 'tags', label: 'Tags', width: 'minmax(180px, 1fr)', sortable: false },
  { key: 'cpu', label: 'CPU', width: '100px', sortable: true },
  { key: 'mem', label: 'Memory', width: '100px', sortable: true },
  { key: 'disk', label: 'Disk', width: '100px', sortable: true },
  { key: 'traffic', label: 'Traffic', width: '100px', sortable: true },
  { key: 'rate', label: 'Rate', width: '80px', sortable: true },
  { key: 'latency', label: 'Latency', width: '136px', sortable: false },
]
...
const sortedNodes = computed(() => {
  ...
  return nodes.sort((a, b) => {
    switch (key) {
      ...
      case 'os': {
        return dir * getOSName(a.os).localeCompare(getOSName(b.os), 'en-US')
      }
    ...
    <div class="min-w-fit w-full flex flex-col gap-1">
      ...
      >
        <div
          ...
          class="relative flex h-16 cursor-pointer flex-col justify-center rounded-lg px-2 shadow-[0_0_4px,0_0_0_1px] shadow-transparent transition-all bg-background/60 hover:bg-background hover:shadow-teal-600/10"
          ...
          <div class="grid gap-2 items-center" :style="gridStyle">
            <template v-for="col in columns" :key="col.key">
              ...
              <div v-if="col.key === 'status'" class="flex justify-center">
                <div class="size-2 rounded-full relative" :class="[node.online ? 'bg-teal-600' : 'bg-red-600']">
                  <div
                    class="animate-ping absolute inset-0 rounded-full opacity-50"
                    :class="[node.online ? 'bg-teal-600' : 'bg-red-600']"
                  />
                </div>
              </div>
              ...
              <div v-else-if="col.key === 'latency'" class="flex items-center">
                <NodePingListCell
                  ...
                  tabindex="0"
                  class="outline-none"
                  :aria-label="`${node.name} realtime latency`"
              ...
              <div v-else-if="col.key === 'rate'">
                <div class="text-[10px] flex flex-col ">
                  <span class="text-teal-600 flex flex-row gap-1 items-center">
                    <Icon icon="tabler:chevron-up" width="12" height="12" />
            ...
            <div class="grid gap-2 items-center justify-center" :style="gridStyle">
              <div class="h-full space-y-1" :style="offlineOverlayContentStyle">
                <div class="text-sm font-semibold truncate">
                  <span class="text-red-500">Offline</span> {{ node.name }}
                </div>
