...
// Reduce performance issues caused by high sampling
...
const regionRates = computed<Map<string, RegionRate>>(() => {
  ...
  // Always use nodesStore.nodes to bypass the earthNodes 60s throttle so rates update in real time
...
// iOS Safari's DOM/style behavior for cobe's internal marker anchors is unstable,
// so overlays are projected and positioned within the component to avoid snapping back to the container's top-left.
...
const markers = computed<Marker[]>(() => {
  return regionClusters.value.map(cluster => ({
    ...
    size: 0, // Don't render the dot
  }))
})

// Converge from each region to the user's current location; fall back to hub topology when no user coordinate is available
...
function buildInitialOptions(): COBEOptions {
  ...
  return {
    ...
    dark: colors.dark,
    diffuse: 1.2,
    mapSamples: 10000, // Number of map sample points, default 16000
...
// When phi has converged/is idle, skip globe.update for the whole frame, zeroing out both the WebGL and overlay position updates
...
function startGlobe() {
  ...
  syncClusterOverlayPositions()
  // A stopped globe has no rotation frames, so the first frame must actively redraw once after the actual size stabilizes.
  requestAnimationFrame(() => {
    updateGlobeFrame()
  })
  // documentVisibility is readable synchronously; useElementVisibility waits for the IntersectionObserver's first callback
  // Start as if "foreground"; if actually invisible, the shouldRender watch will pause on the next frame
  syncRafState()
}

// cobe doesn't clean up the wrapper it creates, so tidy up manually here.
function stopGlobe() {
  pauseRaf()
  globe?.destroy()
  globe = null
...
// Rebuild the globe when the theme changes
