...
onMounted(async () => {
  ...
  try {
    ...
    loadError.value = error instanceof Error ? error.message : 'Failed to load map resources'
  }
  finally {
    loading.value = false
  }
...
<template>
  <div class="relative h-full border-none" content-class="h-full !p-0">
    <!-- Preload flag images to avoid repeated tooltip requests -->
    <div class="hidden">
    ...
    <div class="relative flex h-88 flex-col items-center">
      ...
      <div class="relative flex-1 w-full md:-translate-y-1/6 -translate-y-1/5">
        <Spinner :show="loading" class="h-full w-full" content-class="!bg-transparent">
          ...
          <Empty
            v-else-if="loadError"
            description="Failed to load map resources"
            class="h-full"
          />
        </Spinner>
      </div>
